...
/**
 * SINGLETON CREDIBILITY ANALYZER
 * We create one global instance to ensure that app.py is spawned only once when the server starts.
 */
class CredibilityAnalyzer {
    ...
    async initializePythonServer() {
        ...
        // Log Python output (optional, for debugging)
    ...
    async analyze(text, metadata = {}, attempt = 0) {
        try {
            ...
            // Fallback or default result if needed
            return { confidence: 0.5, details: 'Fallback analysis' };
        }
    }

    // Optional cleanup method if you need to gracefully stop the python process
    cleanup() {
...
// Create a single global instance of the analyzer
const globalCredibilityAnalyzer = new CredibilityAnalyzer();
...
class StreamProcessor {
    constructor(assemblyAI, ws) {
        ...
        this.analyzer = globalCredibilityAnalyzer; // Use the global analyzer
...
const fetchTrendingNews = async () => {
    ...
    try {
        ...
        for (const current of allNews) {
            ...
            if (!isDuplicate) {
                // Analyze the combined text using the global analyzer
                const analysis = await globalCredibilityAnalyzer.analyze(
...
app.post('/api/analyze-article', async (req, res) => {
    ...
    try {
        ...
        // Use global analyzer
        const analysis = await globalCredibilityAnalyzer.analyze(articleText, {
            type: 'article',
            url,
            language
        });
...
app.post('/api/analyze-text', async (req, res) => {
    ...
    try {
        // Use global analyzer
        const analysis = await globalCredibilityAnalyzer.analyze(text, {
            type: 'manual_input',
            language
        });
...
app.post('/api/transcribe-recorded', async (req, res) => {
    ...
    try {
        ...
        // Use global analyzer
        const analysis = await globalCredibilityAnalyzer.analyze(transcript.text, {
            type: 'recorded_video',
            platform,
            url: video_url
        });
...
const gracefulShutdown = async () => {
    ...
    // Clean up any active analyzers if you wish
